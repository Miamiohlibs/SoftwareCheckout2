...
module.exports = class LogQuerier {
  ...
  async getFirstTimestampForUids(entries) {
    const firstTimestampForUids =
      '[.entries | group_by(.uid)| map({uid: .[0].uid, first_message: .[0].message, first_content: .[0].content, first_timestamp: (map(.timestamp) | min), total_entries: length, total_size_kb: ((map(tostring | length) | add) / 1024 | round)}) | sort_by(.first_timestamp)]';
