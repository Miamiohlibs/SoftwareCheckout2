import AdobeApi from '../models/AdobeApi';
// const Throttle = require('../helpers/Throttle');
const { asyncForEach } = require('../helpers/utils');
import _ from 'lodash';
// const logger = require('../services/logger');

class AdobeUserMgmtService {
  constructor(conf) {
  ...
  async getPaginatedResults(container) {
    console.debug('AdobeRepo: starting getPaginatedResults...');
    let allResults = [];
    let lastPage = false;
  ...
  async getGroupMembers(group) {
    console.debug('AdobeRepo: starting getGroupMembers())');
    this.clearQueryConf();
    this.queryConf.url =
    ...
    console.debug('AdobeRepo: getGroupMembers query conf', {
  ...
  async submitActionReqs(reqBodyChunks) {
    try {
      ...
      await asyncForEach(reqBodyChunks, async (data) => {
        ...
        console.debug('AdobeRepo: submitting action with queryConf', {
          content: this.queryConf,
        });
        let res = await this.api.getQueryResults(this.queryConf);
      ...
      console.debug('action summary results', {
  ...
  concatActionResults(data) {
    console.debug('concatActionResults', { content: data });
...
export default AdobeUserMgmtService;
