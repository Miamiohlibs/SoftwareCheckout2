...
The both kinds of logs should be stored outside of the application directory and should be readable and writable by the user running the application. The log locations are defined in the top-level of the appConf.js file. Each of these should be a root-relative file path without a final `/`.

* `logLocation` - e.g. `/opt/SoftwareCheckout/logs/appLogs`
* `statsLogLocation` - Inside the folder defined here, create three subfolders also readable and writable by the user: `dailyStats`, `eachCheckout`, `summaryStats`.

Also set up a cryptographic hash for anonymizing the stats logs.&#x20;
